...
function getWaypoints()
  ...
  for i=1, waypoints.n do
    if waypoints[i].label == "FARMBASE" then      --"FARMBASE"
      BASE.x = waypoints[i].position[1]
      BASE.y = waypoints[i].position[2]
      BASE.z = waypoints[i].position[3]
    elseif waypoints[i].label == "DROPOFF" then   --"DROPOFF"
      DROPOFF.x = waypoints[i].position[1]
      DROPOFF.y = waypoints[i].position[2]
      DROPOFF.z = waypoints[i].position[3]
    elseif waypoints[i].label:find("FARMROW") == 1 then  --"FARMROW LEN0 R0"  R means to the right. go into the negative and you go to the left. Note that 1 equals 3 blocks to the right or with minus to the left.
      local tempTable = {}
      tempTable.x = waypoints[i].position[1]
      tempTable.y = waypoints[i].position[2]
      tempTable.z = waypoints[i].position[3]
      tempTable.len = waypoints[i].label:match("LEN(%d+)")
      tempTable.r = waypoints[i].label:match("R(%d+)")
...
function farm()
  ...
  for i=1, #FARMROWs do
    ...
    local l, r, tx, tz
    if direction == 2 then l,r,tx,tz,ttx,ttz = 4,5,0,-1,1,0 end --2=NORTH
    if direction == 3 then l,r,tx,tz,ttx,ttz = 5,4,0,1,-1,0 end  --3=SOUTH
    if direction == 4 then l,r,tx,tz,ttx,ttz = 3,2,-1,0,0,-1 end --4=WEST
    if direction == 5 then l,r,tx,tz,ttx,ttz = 2,3,1,0,0,1 end  --5=EAST

    
    for x=0, row.r do
     move(row.x+3*x*ttx, row.y, row.z+3*x*ttz)
     drone.use(direction)
      for i=1, row.len do
        move(cx+tx, cy, cz+tz)
        colour("farming")
        drone.use(l)
        drone.use(r)
        if i < tonumber(row.len) then
          drone.use(direction)
        end
      end
     if drone.count(drone.inventorySize()) >= 1 then
       dropoff()
     end
...
--  while docking.dock()~= true do computer.pullSignal(.1) end
  colour("dropping")
  for i=drone.inventorySize(),1,-1 do
